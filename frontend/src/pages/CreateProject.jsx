...
const CreateProject = () => {
  ...
  const handleAddSkillRequirement = () => {
    setSkillRequirements([...skillRequirements, { skill_id: "", employees_requested: 1 }]);
  };
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/projects/", {
        ...
        body: JSON.stringify({
          ...
          priority,
          status,
          manager_id: user.id, // Autofill manager_id with the current user's ID
      ...
      if (response.ok) {
        setSuccess(true);
        setTimeout(() => navigate("/view-projects"), 2000); // Redirect to project list
      } else {
        const data = await response.json();
